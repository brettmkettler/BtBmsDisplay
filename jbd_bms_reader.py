#!/usr/bin/env python3
"""
JBD BMS Reader - Unified implementation combining working connection and parsing logic
"""
...
class JBDBMSReader:
    """JBD BMS Reader using bms_detailed_test.py connection logic"""
    ...
    def connect(self) -> bool:
        """Connect using bms_detailed_test.py logic"""
        try:
    ...
    def read_data(self) -> bool:
        ...
        # JBD BMS commands from bms_detailed_test.py
        commands = [
            {
...
def test_jbd_reader(mac_address: str, track: str):
    """Test the unified JBD BMS reader"""
    print(f"\n=== Testing JBD BMS Reader for {track.upper()} ===")
    ...
    try:
        ...
        if reader.connect():
            ...
            if reader.read_data():
                ...
                if battery:
                    print(f"\n--- {track.upper()} Battery Data ---")
                    print(f"Voltage: {battery.voltage:.2f}V")
                    print(f"Current: {battery.current:.2f}A")
                    print(f"SOC: {battery.soc:.1f}%")
                    print(f"Capacity: {battery.remaining_capacity:.2f}/{battery.total_capacity:.2f}Ah")
...
if __name__ == "__main__":
    # Test with RIGHT track
    success = test_jbd_reader('E0:9F:2A:E4:94:1D', 'right')
